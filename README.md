# UVE STUDIO — 랜딩 사이트 (가칭)

AX 실행 파트너 스튜디오의 홈페이지 + 문의(AX 진단) 페이지. 정적 사이트(HTML/CSS/JS 단일 파일, 빌드 불필요).

> ⚠️ 브랜드명 미확정(G1): "UVE STUDIO (가칭)" 표기. 성과 사례·클라이언트 로고는 전부 **가상 예시**(파일럿 후 실측 교체).
...
assets/
  uve-mark.svg   로고 마크(Vessel) — 그릇 + 상승 스파크
  favicon.svg    파비콘
```
...
- **로고**: Vessel 마크 — U(그릇)에서 스파크가 떠오르는 형태. "교육을 담아 성과로."
...
- [ ] 성과 사례·클라이언트 로고 실측·실명 데이터로 교체 (G2)
- [ ] 브랜드명·도메인 확정 후 "(가칭)" 제거, 메타/OG 태그 보강 (G1)
- [ ] 실제 이미지(제품/현장 사진·일러스트) 확보 — 현재 SVG 아이콘·컬러 타일로 대체
- [ ] 패키지 가격 확정 반영 (G0)
